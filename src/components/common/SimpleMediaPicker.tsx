...
type SimpleMediaPickerOptions = {
    category_name?: string,
    cropped?: boolean
};

export default function SimpleMediaPicker({ category_name, cropped, onSelect }: { onSelect: (image: string) => void } & SimpleMediaPickerOptions) {
    ...
    const search = async () => {
        setSearching(true);
        var r = await MediaManagerService.search({ ...filters, category_name });
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
                            ...
                            <li key={media.id} className=" cursor-pointer flex flex-col sm:flex-row items-start sm:items-center p-3 gap-3 bg-gray-50 rounded-lg shadow-md border border-gray-200 transition hover:shadow-lg duration-150">
                                ...
                                <div className="flex space-x-2 flex-shrink-0">
                                    <Btn variant={'outline'} size={'xs'} asyncClick={async () => {
                                        if (cropped) {
                                            var file = await openCropperUrl(media.media_path, {
                                                format: 'file'
                                            });
                                            if (file instanceof File) {
                                                ...
                                                if (r.success) {
                                                    onSelect(r.data.media_path);
                                                }
                                            }
                                        } else {
                                            onSelect(media.media_path)
                                        }
...
export async function pickImageUrl(options?: SimpleMediaPickerOptions): Promise<string> {
