...
const LazyEditorDialog = lazy(() => import('./components/ExamQuestionGroupEditorDialog'));

export default function ExamQuestionGroupListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await ExamQuestionGroupService.search({ ...filters, organization_id });
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Question Set' : 'Update Question Set',
    ...
    return (
        <AppPage
            title='Question Sets'
            subtitle='Manage question sets which can be assigned to differnt differnt time slots'
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await ExamQuestionGroupService.delete({ id: record.id, organization_id });
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
                                            ...
                                            <Link to={'/exam-management/exam-question-groups/' + record.id}>
