...
export default function ExamEditorDialog({ id, onSuccess, onCancel }: Props) {
    ...
    const getDetail = async () => {
        ...
        setLoading(true);
        var r = await ExamCategoryService.detail({ id, organization_id });
        if (r.success) {
            setForm(r.data);
            setLoading(false);
        }
    ...
    const save = async () => {
        ...
        if (id) {
            r = await ExamCategoryService.update({ ...form, organization_id });
        } else {
            r = await ExamCategoryService.create({ ...form, organization_id });
        }
        if (r.success) {
