...
const LazyEditorDialog = lazy(() => import('./components/ExamCategoryEditorDialog'));

export default function ExamCategoryListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await ExamCategoryService.search({ ...filters, organization_id });
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Test Service Category' : 'Update Test Service Category',
    ...
    return (
        <AppPage
            ...
            subtitle='Manage all exam categories from this section'
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await ExamCategoryService.delete({ id: record.id, organization_id });
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
