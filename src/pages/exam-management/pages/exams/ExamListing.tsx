...
const LazyEditorDialog = lazy(() => import('./components/ExamEditorDialog'));
...
export default function ExamListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await ExamService.search({ ...filters, organization_id });
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    return (
        <AppPage
            title='Exams'
            subtitle='Manage all exams form this section'
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell>{record.exam_category_name}</TableCell>
                                    <TableCell>
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await ExamService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
