...
const LazyEditorDialog = lazy(() => import('./components/ExamSectionEditorDialog'));

export default function ExamSectionListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await ExamSectionService.search({ ...filters, organization_id });
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Section' : 'Update Section',
    ...
    return (
        <AppPage
            title='Exam Section'
            subtitle='Manage all exam Section from this section'
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await ExamSectionService.delete({ id: record.id, organization_id });
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
