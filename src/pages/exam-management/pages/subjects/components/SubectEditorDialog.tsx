...
export default function SubectEditorDialog({ id, onSuccess, onCancel }: Props) {
    ...
    const getDetail = async () => {
        ...
        setLoading(true);
        var r = await SubjectService.detail({ id, organization_id });
        if (r.success) {
            setForm(r.data);
            setLoading(false);
        } else {
    ...
    const save = async () => {
        ...
        if (id) {
            r = await SubjectService.update({ ...form, organization_id });
        } else {
            r = await SubjectService.create({ ...form, organization_id });
        }
        if (r.success) {
