...
const LazyEditorDialog = lazy(() => import('./components/SubectEditorDialog'));

export default function SubectListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await SubjectService.search({ ...filters, organization_id });
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Subject' : 'Update Subject',
    ...
    return (
        <AppPage
            title='Subjects'
            subtitle='Subjects wich will be used in various exams and test series'
                                        ...
                                        {record.organization_id == organization_id && <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await SubjectService.delete({ id: record.id, organization_id });
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
