...
export default function ShipmentsTab({ showFilters, tab }: { tab: string, showFilters: boolean }) {
    ...
    const search = async () => {
        setSeraching(true);
        var r = await UserOrderService.searchShipments(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSeraching(false);
    ...
    useEffect(() => {
        if (tab == 'processing') {
            setFilter('status')(UserOrderStatus.Processing);
        } else if (tab == 'delivered') {
            setFilter('status')(UserOrderStatus.Delivered);
        } else if (tab == 'ready') {
            setFilter('status')(UserOrderStatus.ReadyForShipment);
        }
    }, [tab])
                                ...
                                {paginated.records.map((order) => {
                                    ...
                                    return (<TableRow key={`heading_${order.id}`}>
                                        ...
                                        <TableCell className="font-medium">
                                            <Link className='text-blue-600 underline' to={'/orders/' + order.order_internal_reference_number}>{order.order_internal_reference_number}</Link>
                                        </TableCell>
                                        <TableCell className="font-medium">
                                            <Link className='text-blue-600 underline' to={'/invoices/' + order.invoice_internal_reference_number}>{order.invoice_internal_reference_number}</Link>
                                        </TableCell>
                                        ...
                                        <TableCell className="text-right space-x-2">
                                            ...
                                            <div className='flex flex-row items-center gap-3 justify-end'>
                                                <DownloadInvoiceBtn internal_reference_number={order.invoice_internal_reference_number} />
                                                <Btn
                                                    size="sm"
                                                    variant="outline"
                                                    onClick={() => navigate(`/shipments/${order.internal_reference_number}`)}
                                                >
