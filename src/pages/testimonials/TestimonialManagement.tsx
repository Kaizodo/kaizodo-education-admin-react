...
export default function TestimonialManagement() {
    ...
    return (
        <AppPage title='Testimonials' subtitle='Manage testimonials which will shown on website' actions={<Btn onClick={() => openEditor()}><FaPlus />Add New</Btn>}>
            <AppCard>
                                    ...
                                    <TableCell>
                                        <div className='flex flex-row'>
                                            {Array.from({ length: 5 }, (_, i) =>
                                                i < record.rating ? <FaStar key={i} /> : <FaRegStar key={i} />
                                            )}
                                        </div>
                                    </TableCell>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await TestimonialService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
