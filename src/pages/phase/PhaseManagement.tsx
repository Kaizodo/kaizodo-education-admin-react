...
const LazyEditorDialog = lazy(() => import('./components/PhaseEditorDialog'));
...
export default function PhaseManagement() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await PhaseService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Phase' : 'Update Phase',
            maxWidth: 500,
    ...
    return (
        <AppPage
            ...
            subtitle='Manage project phases which will help organize project deployment'
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell>
                                        <div>
                                            <div className='h-5 w-20 rounded-sm' style={{
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await PhaseService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
