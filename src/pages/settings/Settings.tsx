...
const SettingsManagement = () => {
    ...
    const load = async () => {
        ...
        setLoading(true);
        var r = await StoreService.loadSettings(context.organization.id);
        if (r.success) {
            setForm(r.data);
        }
        setLoading(false);
    ...
    const save = async () => {
        setSaving(true);
        var r = await StoreService.saveSettings({ ...form, organization_id: context.organization.id });
        if (r.success) {
    ...
    return (
        <AppPage title='Settings' subtitle='Manage school and system settings' actions={<div className='pe-6'><Btn onClick={save} loading={saving}>Save Settings</Btn></div>}>
            ...
            >
                ...
                />}
                {!!context?.organization?.id && loading && <CenterLoading className='relative h-[400px]' />}
                {!!context?.organization?.id && !loading && <>
