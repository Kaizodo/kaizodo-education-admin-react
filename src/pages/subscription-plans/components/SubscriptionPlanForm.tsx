...
import { CustomSubscriptionPlanBuilder } from './CustomSubscriptionPlanBuilder';
...
import FeatureComparePoints from './FeatureComparePoints';
import ContentCards from './ContentCards';
import { ReferralTab } from './ReferralTab';
import { MarketingMaterialTab } from './MarketingMaterialTab';
...
export default function SubscriptionPlanForm({ id, custom = false, onSuccess, inModal = false }: { id?: number, custom?: boolean, onSuccess?: (data: any) => void, inModal?: boolean }) {
    ...
    const save = async () => {
        ...
        var r = await SubscriptionPlanService.create({ ...rest, custom, marketing_material_ids: marketing_materials.map(i => i.id) });
        if (r.success) {
    ...
    const ui = (<>
        ...
        {!loading && <AppCard contentClassName="p-2"
            title={form.name ?? 'Subscription Plan'}
            subtitle={form.description ?? 'Enter subscription plan description '}
            actions={!inModal && <div className='me-6'><Btn loading={saving} onClick={save} size={'sm'}>Save Details</Btn></div>}>
            <Tabs defaultValue="plan">
                <TabsList>
                    <TabsTrigger value="plan">Plan Details</TabsTrigger>
                    {!custom && <TabsTrigger value="compare">Feature Comparison</TabsTrigger>}
                    ...
                    {!custom && <TabsTrigger value="terms">Terms & Documents (NDA)</TabsTrigger>}
                </TabsList>
                <TabsContent value="plan">
                    <div className='grid grid-cols-3 gap-4  w-full'>
                        <div className=' p-4   flex-1 space-y-3'>
                            ...
                            <div className='grid grid-cols-2 gap-3'>
                                <TextField type='number' value={form?.pricing?.[0]?.price ?? 0} onChange={setValue('pricing[0].price')}>Price</TextField>
                                <TextField type='number' value={form?.pricing?.[0]?.duration_days ?? 0} onChange={setValue('pricing[0].duration_days')}>Duration Days</TextField>
                            </div>
                            {!custom && <div className='border rounded-lg overflow-hidden'>
                                <Table>
                                    <TableHeader>
                                        <TableRow>
                ...
                <TabsContent value="terms">
                    <TermsTab state={form} setValue={setValue} />
                </TabsContent>
            </Tabs>
        </AppCard>}
    </>);
