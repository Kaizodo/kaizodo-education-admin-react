...
export default function SubscriptionModules() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await ReferralSchemeService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Referral Scheme' : 'Update Referral Scheme',
            maxWidth: 500,
    ...
    return (
        <AppPage
            ...
            subtitle='Manage time limited referral schemes'
                                ...
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell>
                                        <div className='flex flex-col'>
                                            ...
                                            <span>{moment(record.valid_to).format('DD MMM, Y LT')}</span>
                                        </div>
                                    </TableCell>
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await ReferralSchemeService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
