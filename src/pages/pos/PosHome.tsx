...
import { Search, User, Calendar, X, Plus, Edit, Tag, CreditCard, ClipboardList, Save, FileSignature, Package } from 'lucide-react';
import { LuBadgePercent, LuChevronDown, LuCircle, LuCircleCheck, LuMinus, LuPackage, LuPlus, LuUserPlus, LuUserX, LuX } from 'react-icons/lu';
...
export default function PosHome() {
                        ...
                        <TableRow key={product.id}>
                            ...
                            <TableCell>
                                <div className="w-[150px] flex flex-row items-center gap-2">
                                    <Btn size={'sm'} variant={'outline'} onClick={() => product.quantity <= 1 ? setValue(`products[id:${product.id}]-`)() : setValue(`products[id:${product.id}].quantity`)(Number(product.quantity) - 1)}><LuMinus /></Btn>
                                    <div className='flex-1'>
                                        <TextField no_arrows={true} className="text-center " value={product.quantity} onChange={setValue(`products[id:${product.id}].quantity`)} type='number' placeholder='Quantity' />
                                    </div>
                                    <Btn size={'sm'} variant={'outline'} onClick={() => setValue(`products[id:${product.id}].quantity`)((Number(product.quantity) + 1))}><LuPlus /></Btn>
                                </div>
                            </TableCell>
    ...
    const PriceSummary = () => {
        ...
        const save = async (is_draft: boolean, is_proforma = false, has_project = false) => {
            var { discount_plan, customer, ...rest } = form;
            var r = await PosService.generateInvoice({
                ...rest,
                invoice_date: moment(rest.invoice_date).format('Y-MM-DD'),
                due_date: moment(rest.due_date).format('Y-MM-DD'),
                is_draft,
                is_proforma,
                has_project,
                user_id: customer?.id,
        ...
        return (
            <div className="bg-white p-6 rounded-xl shadow-2xl h-fit sticky top-4 border-2 border-indigo-100">
                <h4 className="text-xl font-bold mb-4 text-gray-800 flex items-center"><span className='h-8 w-8 bg-primary text-white flex items-center justify-center me-2 rounded-full'>{context.settings.currency_symbol}</span>Payment & Summary</h4>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {PaymentMethodArray.map(method => (
                            <button
                                ...
                                className={`text-sm px-2 py-1  rounded-xl transition-colors  min-w-16 ${form.payment_method === method.id
                ...
                <label className="flex items-center gap-2 bg-sky-50 border text-sky-800 px-3 py-2 rounded-lg select-none hover:bg-sky-100 cursor-pointer">
                    <Checkbox checked={form.has_delivery} onCheckedChange={setValue('has_delivery')} />
                    <span className="font-medium">Delivery to customer</span>
                </label>
                ...
                <Btn
                    asyncClick={async () => save(false, false, true)}
                    disabled={form.products.length === 0}
                    className="w-full mt-6 h-auto  bg-indigo-600 text-white  text-xl rounded-xl shadow-xl shadow-indigo-400/50 hover:bg-indigo-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition"
                >
                    <div className='flex flex-col '>
                        {!!form.has_project && <span>Update Job Card</span>}
                        {!form.has_project && <span>Create Job Card</span>}
                        <span className='text-xs'>Payment can be collected after</span>
                    </div>
                </Btn>
                ...
                <Btn
                    asyncClick={async () => save(false, false)}
                    disabled={form.products.length === 0}
                    className="w-full mt-6 py-4 bg-green-600 text-white font-bold text-xl rounded-xl shadow-xl shadow-green-400/50 hover:bg-green-700 disabled:bg-gray-400 disabled:cursor-not-allowed transition h-16"
                >
                    Finalize & Pay
                </Btn>
                ...
                <div className="grid grid-cols-2 gap-3 mt-3">
                    <Btn
                        asyncClick={async () => await save(true, false)}
                    ...
                    <Btn
                        ...
                        asyncClick={async () => await save(false, true)}
                    >
