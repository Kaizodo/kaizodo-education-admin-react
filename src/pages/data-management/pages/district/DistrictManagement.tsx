...
const LazyEditorDalog = lazy(() => import('./components/DistrictEditorDailog'));
...
export default function DistrictManagement() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await DistrictService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: id ? 'Update District' : 'Add District',
    ...
    return (
        <AppPage title='District' subtitle='Manage district-level administrative data' actions={<Btn onClick={() => openEditor()}><FaPlus />Add New</Btn>}>
            <AppCard>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await DistrictService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
