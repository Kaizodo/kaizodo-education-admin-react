...
const LazyEditorDalog = lazy(() => import('./components/LocalityEditorDailog'));
...
export default function LocalityManagement() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await LocalityService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: id ? 'Update Locality' : 'Add Locality',
    ...
    return (
        <AppPage title='Locality' subtitle='Manage specific local areas within cities' actions={<Btn onClick={() => openEditor()}><FaPlus />Add New</Btn>}>
            <AppCard>
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell className="font-medium">{record.city_name}</TableCell>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await LocalityService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
