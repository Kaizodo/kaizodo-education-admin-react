...
export default function CityEditorDailog({ id, onSuccess, onCancel }: Props) {
    ...
    const getDetail = async () => {
        ...
        setLoading(true);
        var r = await CityService.detail(id);
        if (r.success) {
            setForm(r.data);
            setLoading(false);
        }
    ...
    const save = async () => {
        ...
        if (id) {
            r = await CityService.update(form);
        } else {
            r = await CityService.create(form);
        }
