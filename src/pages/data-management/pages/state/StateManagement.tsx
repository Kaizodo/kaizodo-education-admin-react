...
const LazyEditorDalog = lazy(() => import('./components/StateEditorDailog'));
...
export default function StateManagement() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await StateService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: id ? 'Update State' : 'Add State',
    ...
    return (
        <AppPage title='State' subtitle='Manage state information across different regions' actions={<Btn onClick={() => openEditor()}><FaPlus />Add New</Btn>}>
            <AppCard>
