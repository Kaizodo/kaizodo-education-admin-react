...
const LazyEditorDalog = lazy(() => import('./components/PermissionGroupEditorDialog'));

export default function PermissionGroupListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await PermissionGroupService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: id ? 'Edit Permission Group' : 'Add Permission Group',
    ...
    return (
        <AppPage title='Permission Groups' subtitle='Manage permission groups used to categorize related permissions for better access control' >

            <AppCard title='Group List' actions={<div className='me-4'><Btn size={'sm'} onClick={() => openEditor()}><FaPlus />Add New</Btn></div>}>
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await PermissionGroupService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
