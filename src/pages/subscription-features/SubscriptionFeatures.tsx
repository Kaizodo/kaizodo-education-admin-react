...
const LazyEditorDialog = lazy(() => import('./components/SubscriptionFeatureEditorDialog'));
const LazySubscriptionFeatureContentEditor = lazy(() => import('./components/SubscriptionFeatureContentEditor'));
...
export default function SubscriptionFeatures() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await SubscriptionFeatureService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Subscription Feature' : 'Update Subscription Feature',
            maxWidth: 500,
    ...
    const openContentEditor = async (record: any) => {
        Modal.show({
            ...
            maxWidth: '95%',
            content: () => <Suspense fallback={<CenterLoading className='h-[400px] relative' />}>
                <LazySubscriptionFeatureContentEditor id={record.id} />
            </Suspense>
        });
    }
    ...
    return (
        <AppPage
            ...
            subtitle='Predefine subscription modules which will be used in various sections'
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell>{record.subscription_module_name}</TableCell>
                                    <TableCell>{record.name}</TableCell>
                                    <TableCell>{record.code}</TableCell>
                                    <TableCell>₹{record.price}</TableCell>
                                    <TableCell>{record.description}</TableCell>
                                    <TableCell className='text-xs italic'>{'/module/' + record.subscription_module_slug + '/' + record.slug}</TableCell>
                                    <TableCell>
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await SubscriptionFeatureService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
