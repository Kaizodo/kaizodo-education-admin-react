...
export default function SubscriptionFeatureContentEditor({ id }: { id: number }) {
    ...
    const save = async () => {
        setSaving(true);
        var r = await SubscriptionFeatureService.saveContent(form);
        if (r.success) {
    ...
    const load = async () => {
        setLoading(true);
        var r = await SubscriptionFeatureService.loadContent(id);
        if (r.success) {
