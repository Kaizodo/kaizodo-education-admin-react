...
import { DiscountBy, getDiscountTypeName } from '@/data/Common';

const LazyEditorDialog = lazy(() => import('./components/DiscountPlanEditorDialog'));
...
export default function DiscountPlans() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await DiscountPlanService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Discount' : 'Update Discount',
            maxWidth: 500,
    ...
    return (
        <AppPage
            ...
            subtitle='Manage discount plans which will be applied to premium packages.'
                    ...
                    {!searching && <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>Discount By</TableHead>
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell>{getDiscountTypeName(record.discount_type)}</TableCell>
                                    <TableCell>{moment(record.valid_from).format('DD MMM, Y')} - {moment(record.valid_to).format('DD MMM, Y')}</TableCell>
                                    <TableCell>{record.discount_by == DiscountBy.Amount ? 'Amount' : 'Percentage'}</TableCell>
                                    <TableCell>{record.discount_by == DiscountBy.Amount ? '₹' + record.discount_amount : record.discount_percentage + '%'}</TableCell>
                                    <TableCell>{record.quantity_used} / {record.quantity}</TableCell>
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await DiscountPlanService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
