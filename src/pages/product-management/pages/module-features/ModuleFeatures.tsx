...
const LazyEditorDialog = lazy(() => import('./components/ModuleFeatureEditorDialog'));
const LazyModuleFeatureContentEditor = lazy(() => import('./components/ModuleFeatureContentEditor'));
...
export default function ModuleFeatures() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await ModuleFeatureService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Module Feature' : 'Update Module Feature',
            maxWidth: 500,
    ...
    const openContentEditor = async (record: any) => {
        Modal.show({
            ...
            maxWidth: '95%',
            content: () => <Suspense fallback={<CenterLoading className='h-[400px] relative' />}>
                <LazyModuleFeatureContentEditor id={record.id} />
            </Suspense>
        });
    }
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell className='text-xs italic'>{'/module/' + record.module_slug + '/' + record.slug}</TableCell>
                                    <TableCell>
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await ModuleFeatureService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
