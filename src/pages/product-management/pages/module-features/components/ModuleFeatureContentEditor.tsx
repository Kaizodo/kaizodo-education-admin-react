...
export default function ModuleFeatureContentEditor({ id }: { id: number }) {
    ...
    const save = async () => {
        setSaving(true);
        var r = await ModuleFeatureService.saveContent(form);
        if (r.success) {
    ...
    const load = async () => {
        setLoading(true);
        var r = await ModuleFeatureService.loadContent(id);
        if (r.success) {
