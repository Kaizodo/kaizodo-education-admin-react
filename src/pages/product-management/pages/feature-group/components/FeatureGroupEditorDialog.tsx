...
export default function FeatureGroupEditorDialog({ id, onSuccess, onCancel }: Props) {
    ...
    const getDetail = async () => {
        ...
        setLoading(true);
        var r = await FeatureGroupService.detail(id);
        if (r.success) {
            setForm(r.data);
            setLoading(false);
        }
    ...
    const save = async () => {
        ...
        if (id) {
            r = await FeatureGroupService.update(form);
        } else {
            r = await FeatureGroupService.create(form);
        }
        if (r.success) {
