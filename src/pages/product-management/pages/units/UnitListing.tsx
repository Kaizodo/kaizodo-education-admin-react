...
const LazyEditorDialog = lazy(() => import('./components/UnitEditorDialog'));
...
export default function UnitListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await UnitService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    return (
        <AppPage
            title='Product Units'
            subtitle='Manage product units form this section'
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await UnitService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
