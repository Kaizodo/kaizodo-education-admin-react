...
export default function TaxComponentEditorDialog({ id, onSuccess, onCancel }: Props) {
    ...
    const getDetail = async () => {
        ...
        setLoading(true);
        var r = await TaxComponentService.detail(id);
        if (r.success) {
            setForm(r.data);
            setLoading(false);
        }
    ...
    const save = async () => {
        ...
        if (id) {
            r = await TaxComponentService.update(form);
        } else {
            r = await TaxComponentService.create(form);
        }
        if (r.success) {
