...
const LazyEditorDialog = lazy(() => import('./components/TaxComponentEditorDialog'));

export default function TaxComponentListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await TaxComponentService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Tax Component' : 'Update Component',
    ...
    return (
        <AppPage
            ...
            subtitle='Manage various tax components'
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await TaxComponentService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
