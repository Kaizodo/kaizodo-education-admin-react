...
const LazyEditorDialog = lazy(() => import('./components/ModuleEditorDialog'));
...
export default function Modules() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await ModuleService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Module' : 'Update Module',
    ...
    return (
        <AppPage
            title='Modules'
            subtitle='Predefine Modules which will be used in various sections'
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell className='text-xs italic'>{'/module/' + record.slug}</TableCell>
                                    <TableCell>
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await ModuleService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
