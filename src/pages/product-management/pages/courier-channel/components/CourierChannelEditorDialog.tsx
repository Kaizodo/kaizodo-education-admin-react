...
export default function CourierChannelEditorDialog({ id, onSuccess, onCancel }: Props) {
    ...
    const getDetail = async () => {
        ...
        setLoading(true);
        var r = await CourierChannelService.detail(id);
        if (r.success) {
            setForm(r.data);
            setLoading(false);
        }
    ...
    const save = async () => {
        ...
        if (id) {
            r = await CourierChannelService.update(form);
        } else {
            r = await CourierChannelService.create(form);
        }
        if (r.success) {
    ...
    return (
        <>
            <ModalBody className='relative'>
                ...
                <TextField value={form.code} onChange={setValue('code')} placeholder='Enter code'>Code / Channel ID</TextField>
                <Radio value={form.is_standard_charge} onChange={setValue('is_standard_charge')} options={YesNoArray}>use standard charges ?</Radio>
                {!!form.is_standard_charge && <TextField value={form.standard_charge} onChange={setValue('standard_charge')} placeholder='Enter price'>Standard Charges</TextField>}
            </ModalBody>
