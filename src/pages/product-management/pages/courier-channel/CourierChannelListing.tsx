...
const LazyEditorDialog = lazy(() => import('./components/CourierChannelEditorDialog'));

export default function CourierChannelListing() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await CourierChannelService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Courier Partner' : 'Update Courier Partner',
    ...
    return (
        <AppPage
            ...
            subtitle='Manage courier parnters to easly handle product delivery'
                    ...
                    {!searching && <Table>
                        <TableHeader>
                            <TableRow>
                                ...
                                <TableHead>Standard Charges</TableHead>
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell>{record.is_standard_charge ? '₹' + record.standard_charge : 'Dynamic Pricing'}</TableCell>
                                    <TableCell>
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await CourierChannelService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
