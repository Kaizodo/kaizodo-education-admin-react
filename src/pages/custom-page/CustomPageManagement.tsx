...
export default function CustomPageManagement() {
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: id ? 'Update Custom Page' : 'Add Custom Page',
            maxWidth: '1000px',
    ...
    return (
        <AppPage title='Custom Page Management' subtitle='Manage custom page which will shown on website' actions={<Btn onClick={() => openEditor()}><FaPlus />Add New</Btn>}>
            <AppCard>
                        ...
                        <TableBody>
                            {paginated.records.map((record) => (
                                <TableRow key={record.id}>
                                    ...
                                    <TableCell  >/page/{record.slug}</TableCell>
                                    <TableCell>
                                        <div className="flex gap-2">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await CustomPageService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
