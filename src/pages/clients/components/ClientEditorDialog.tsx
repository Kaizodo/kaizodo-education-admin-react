...
export default function ClientEditorDialog({ id, mobile, onSuccess, onCancel }: Props) {
    ...
    const getDetail = async () => {
        ...
        setLoading(true);
        var r = await ClientService.detail(id);
        if (r.success) {
            setForm(r.data);
            setLoading(false);
        }
    ...
    const save = async () => {
        ...
        if (id) {
            r = await ClientService.update(form);
        } else {
            r = await ClientService.create(form);
        }
        if (r.success) {
