...
const LazyEditorDalog = lazy(() => import('./components/GalleryEditorDailog'));
...
export default function GalleryManagement() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await GalleryService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: id ? 'Update Gallery' : 'Add Gallery',
            maxWidth: '1000px',
    ...
    return (
        <AppPage title='Gallery Management' subtitle='Manage gallery which will shown on website' actions={<Btn onClick={() => openEditor()}><FaPlus />Add New</Btn>}>
            <AppCard>
                                        ...
                                        <div className="flex items-center gap-3">
                                            <Avatar className="h-10 w-10">
                                                ...
                                                <AvatarFallback className="bg-blue-100 text-blue-600 font-semibold">
                                                    {record.first_name.split(' ').map((n: any) => n[0]).join('')}
                                                </AvatarFallback>
                                            </Avatar>
                                            <div>
                                        ...
                                        <div className="flex gap-2">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await GalleryService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
