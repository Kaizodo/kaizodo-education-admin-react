...
const LazyEditorDialog = lazy(() => import('./components/PhaseStepEditorDialog'));
...
export default function PhaseManagement() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await PhaseStepService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Phase Step' : 'Update Phase Step',
            maxWidth: 500,
    ...
    return (
        <AppPage
            title='Project Phases Steps'
            subtitle='Manage project phase steps which will help gather update from assigned team on project phase'
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await PhaseStepService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
