...
const LazyEditorDialog = lazy(() => import('./components/LeadSourceEditorDialog'));
...
export default function LeadSources() {
    ...
    const search = async () => {
        setSearching(true);
        var r = await LeadSourceService.search(filters);
        if (r.success) {
            setPaginated(r.data);
        }
        setSearching(false);
    ...
    const openEditor = async (id?: number) => {
        const modal_id = Modal.show({
            title: !id ? 'Add Lead Source' : 'Update Lead Source',
            maxWidth: 500,
    ...
    return (
        <AppPage
            title='Lead Sources'
            subtitle='Manage lead sources which will help tag where lead is coming from'
                                        ...
                                        <div className="flex gap-2 justify-end">
                                            ...
                                            <Btn onClick={() => {
                                                msg.confirm('Delete ' + record.name, 'Are you sure you want to delete ' + record.name + '? this action cannot be undone.', {
                                                    onConfirm: async () => {
                                                        var r = await LeadSourceService.delete(record.id);
                                                        if (r.success) {
                                                            search();
                                                        }
                                                        return r.success;
